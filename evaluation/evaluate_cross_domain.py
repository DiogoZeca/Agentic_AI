#!/usr/bin/env python3
"""
evaluate_cross_domain.py — 3-phase cross-domain evaluation: Google-trained model on a new dataset.
...
------
  ...
  Phase 1 — Domain shift (PSI)
      Population Stability Index for all model features, comparing the
      Google Cluster 2011 training distribution to the target dataset.
      Skipped with a warning if the Google features parquet is not found.

  Phase 2 — Model evaluation
      Runs spike_feature_engineer.engineer() on the target data (domain-local
...
Usage (from repo root):
  python evaluation/evaluate_cross_domain.py --agg PATH [--artifacts-dir DIR] [--output DIR]
"""
...
def _eda_warnings(eda_df: pd.DataFrame) -> list[str]:
    ...
    if n_idle_warn:
        warnings.append(
            ...
            f"Per-machine p95 features may cluster at 0 during idle."
        )
    ...
    if n_biz_warn:
        warnings.append(
            ...
            "Hour features (hour_sin/cos) will dominate if the target dataset has strong business-hours patterns."
        )
...
def _parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(
        description = "3-phase cross-domain evaluation of the spike model on a new dataset.",
    )
    p.add_argument(
        "--agg",
        type    = Path,
        default = None,
        metavar = "PATH",
        help    = "Path to cluster_agg.parquet for the target domain (required).",
    )
    p.add_argument(
        "--artifacts-dir",
        dest    = "artifacts_dir",
    ...
    p.add_argument(
        "--output",
        type    = Path,
        default = Path("data/cross_domain_eval"),
        metavar = "DIR",
...
if __name__ == "__main__":
    ...
    if args.agg is None or not args.agg.exists():
        log.error("Target cluster_agg.parquet not found: %s  — pass --agg PATH to your domain parquet.", args.agg)
        sys.exit(1)
