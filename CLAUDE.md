...
Agentic_AI/
├── CLAUDE.md              ← this file
├── session-notes.md       ← full decision log and phase history
├── docker-compose.yml     ← compose file (placeholder — not yet updated for new pipeline)
└── AIModel/               ← all application code
    ├── Dockerfile          ← placeholder — to be updated for new pipeline
    ├── Dockerfile.test     ← placeholder — to be updated for new pipeline
...
Two models trained on the same feature set:
...
- **Monotone constraints** — enabled for binary model only; disabled for `multi:softprob` (undefined semantics)
...
| 15m binary | pending (crashed — restart needed) | — |

Per-class on test: `no_spike`=0.965, `moderate`=0.278, `severe`=0.324.
